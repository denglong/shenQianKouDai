...
+ (void)initialize
{
    ...
    appearance.alertBackgroundColor = [UIColor clearColor];
    appearance.titleFont            = [UIFont systemFontOfSize:13];
    appearance.messageColor         = [UIColor lightGrayColor];//[UIColor colorWithRed:0.3f green:0.3f blue:0.3f alpha:1.0f];
    appearance.messageFont          = [UIFont systemFontOfSize:14];
    appearance.buttonsTextColor     = [UIColor lightGrayColor];//[UIColor colorWithRed:0.5f green:0.5f blue:0.5f alpha:1.0f];
    appearance.buttonsFont          = [UIFont systemFontOfSize:14];
    appearance.separatorsLinesColor = [UIColor lightGrayColor];//[UIColor colorWithRed:0.5f green:0.5f blue:0.5f alpha:0.5f];
    appearance.tapToCloseFont       = [UIFont systemFontOfSize:10];
    appearance.tapToCloseColor      = [UIColor lightGrayColor];//[UIColor colorWithRed:0.5f green:0.5f blue:0.5f alpha:1.0f];
    appearance.tapToCloseText       = @"";
}
...
#ifdef DEBUG
        DLog(@"TAlertViewButtonsAlignHorizontal cant be used with more than %i", (int)TAlertViewHorizontalButtonCountMax);
#endif
        _buttonsAlign = TAlertViewButtonsAlignVertical;
    } else {
...
-(void)showAsMessage
{
    ...
    bgView = [[UIView alloc] initWithFrame:CGRectMake(0, -78, self.frame.size.width, 78)];
    bgView.backgroundColor = [UIColor colorWithRed:0/255.0 green:0/255.0 blue:0/255.0 alpha:0.8];
    ...
    [UIView animateWithDuration:0.4f
                     ...
                     animations:^{
                         _alertView.center = CGPointMake(CGRectGetMidX(self.frame), CGRectGetMinY(self.frame) + _alertView.frame.size.height/2.0f);
                         bgView.frame = CGRectMake(0, 0, self.frame.size.width, 78);
                     }
                     completion:^(BOOL finished) { }];
...
-(void)hide
{
    ...
    if (_alertDisplayStyle == TAlertViewDisplayStyleDynamics) {
        ...
        [UIView animateWithDuration:0.4f
                         ...
                         animations:^{
                             self.alpha = 5.0f;
                             _alertView.center = CGPointMake(CGRectGetMidX(self.frame), CGRectGetMinY(self.frame) - _alertView.frame.size.height/2.0f - 40);

                             bgView.frame = CGRectMake(0, -78, self.frame.size.width, 0);
                         } completion:^(BOOL finished) {
                             [bgView removeFromSuperview];
                             [self removeFromSuperview];
                         }];
...
//    }
    ...
    //添加弹框login
    UIImageView *loginImage = [[UIImageView alloc] initWithFrame:CGRectMake(20, 25, 40, 40)];
    loginImage.image = [UIImage imageNamed:@"icon"];
    [_alertView addSubview:loginImage];
    
    //下边小白条
    UIImageView *downImage = [[UIImageView alloc] initWithFrame:CGRectMake(_alertView.frame.size.width/2 - 15, 70, 30, 4)];
    downImage.image = [UIImage imageNamed:@"messageDownLine"];
    [_alertView addSubview:downImage];
    
    _messageLabel = [[UILabel alloc] initWithFrame:CGRectMake(insideHorisontalMargin + 40, yPos, aletViewFrame.size.width - 2*insideHorisontalMargin, 0)];
    ...
    UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(_messageLabel.frame.origin.x + 5, 25, _messageLabel.frame.size.width + 30, 20)];
    ...
    if (_buttonsTexts && [_buttonsTexts count]) {
        ...
        //点击消失--------------------add denglong
        UIView *buttonView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, 75)];
        UITapGestureRecognizer *buttonTapGestureRecognizer = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(buttonTapped:)];
        [buttonView addGestureRecognizer:buttonTapGestureRecognizer];
        [_alertView addSubview:buttonView];
        [_buttonsViews addObject:buttonView];
...
-(void)addToSuperview
{
    ...
    if ([appDelegate respondsToSelector:@selector(window)]) {
        ...
        self.frame = CGRectMake(0, 0, window.frame.size.width, 78);
        [window addSubview:self];
    }
}

-(BOOL)canBecomeFirstResponder {
    [NSTimer scheduledTimerWithTimeInterval:5 target:self selector:@selector(removeAlert) userInfo:nil repeats:NO];
...
-(void)buttonTapped:(UITapGestureRecognizer*)tapGestureRecognizer
{
    ...
    [self.delegate clickheaderTitleAction];
...
//    labelRect.size.height = newRect.size.height;
    label.frame = CGRectMake(labelRect.origin.x, 45, self.frame.size.width - labelRect.origin.x * 2, 20);//labelRect;
    label.text = text;
}
