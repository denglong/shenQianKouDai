...
- (void)awakeFromNib {
    ...
    [self.goodImage mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(window).offset(0);
        make.right.equalTo(window).offset(0);
        make.height.equalTo(@(viewWidth*0.75));
...
-(void)setCommodity:(Commodity *)commodity
{
    ...
    if ([myInfo.userType integerValue] == 1) {
        self.commodityPricelabel.text = [NSString stringWithFormat:@"￥%.2f", [commodity.vipPrice floatValue]];
        
        NSDictionary *attribtDic = @{NSStrikethroughStyleAttributeName: [NSNumber numberWithInteger:NSUnderlineStyleSingle]};
        NSMutableAttributedString *attribtStr = [[NSMutableAttributedString alloc]initWithString:[NSString stringWithFormat:@"￥%.2f", commodity.goodsPrice] attributes:attribtDic];
        self.vipPrice.attributedText = attribtStr;
    }
    else
    {
    self.commodityPricelabel.text = [NSString stringWithFormat:@"￥%.2f",commodity.goodsPrice];
        NSDictionary *attribtDic = @{NSStrikethroughStyleAttributeName: [NSNumber numberWithInteger:NSUnderlineStyleSingle]};
        NSMutableAttributedString *attribtStr = [[NSMutableAttributedString alloc]initWithString:[NSString stringWithFormat:@"VIP ￥%.2f", [commodity.vipPrice floatValue]] attributes:attribtDic];
        self.vipPrice.attributedText = attribtStr;
    }
