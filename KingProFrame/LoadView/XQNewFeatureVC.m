        ...
        for (int i = 0; i<self.imagesNameArray.count; i++) {
            ...
            if (i == self.imagesNameArray.count - 1) {
                if (!self.completeBtn) {
                    ...
                    completeBtn.backgroundColor     = self.completeBtnColor?self.completeBtnColor:[UIColor clearColor];
                    completeBtn.layer.cornerRadius  = self.completeBtnCornerRadius?self.completeBtnCornerRadius:20;
                    completeBtn.layer.masksToBounds = YES;
                    completeBtn.layer.borderWidth = 1;
                    completeBtn.layer.borderColor = [[UIColor whiteColor] CGColor];
                    [completeBtn addTarget:self action:@selector(completeBtnClick) forControlEvents:UIControlEventTouchUpInside];
                    [completeBtn setTitle:self.completeBtnTitle?self.completeBtnTitle:@"立即体验" forState:UIControlStateNormal];
