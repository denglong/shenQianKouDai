...
- (void)createViewData {
    ...
    if (addId) {
        orderPrice = [NSString stringWithFormat:@"%@", [orderInfoDic objectForKey:@"goodsTotalPrice"]];
    }
    else
    {
    ...
    if (![orderInfoDic[@"membersAddress"] isKindOfClass:[NSNull class]]) {
            ...
            case 1:
            {
                ...
                else
                {
                    ...
                    if (goodsList.count > 0) {
                        ...
                        NSString *urlStr = [NSString stringWithFormat:@"%@?x-oss-process=image/resize,m_lfit,h_500,w_500", [orderDic objectForKey:@"goodsPic"]];
                        ...
                        if ([myInfo.userType integerValue] == 1) {
                            cell.goodsPriceLabel.text = [NSString stringWithFormat:@"￥%.2f", [[orderDic objectForKey:@"vipPrice"] floatValue]];
                        }
                        else
                        {
...
//                    cell.goodsName.attributedText = mutaString;
            ...
            case 3:
            {
                if (indexPath.row == 0)
                {
                    ...
                    cell.allTotle.text = [NSString stringWithFormat:@"¥%.2f", [[orderInfoDic objectForKey:@"goodsTotalPrice"] floatValue]];
...
-(void)getMyOrderDataReq:(NSString *)coupId{
    ...
    NSDictionary *params = @{@"goodsList":goodsList, @"totalPrice":cartModel.goodsPrice, @"couponId":coupId, @"shippingType":shippingType};
...
-(IBAction)submitOrders:(id)sender{
    ...
    else
    {
        ...
        paramsDic=@{@"addressId":myAddressId,//收货地址 21:默认 龙记观澜山
                    ...
                    @"shippingFee":shippingF,//配送费
                    @"shippingType":shippingType,//配送类型
                    @"shippingTime":shippingTime?shippingTime:@"",//预约配送时间
                    @"remark":remark?remark:@"",//订单备注
                    @"orderAmount":orderP?orderP:@"",
                    @"shopAddress":@""};//应付款金额
    }
