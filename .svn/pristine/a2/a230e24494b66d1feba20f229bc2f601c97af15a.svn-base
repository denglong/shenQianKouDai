...
#define kSemiModalDismissNewButtonTag         10005
...
-(UIImageView*)kn_addOrUpdateParentScreenshotInView:(UIView*)screenshotContainer {
    ...
    UIImage *image = [self applyBlurRadius:0 toImage:image1];
    image = [self applyBlurRadius:2 toImage:image];
    UIImageView* screenshot = (id) [screenshotContainer viewWithTag:kSemiModalScreenshotTag];
	if (screenshot) {
		screenshot.image = image;
	}
	else {
		screenshot = [[UIImageView alloc] init];
        screenshot.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height);
        screenshot.image = image;
...
//                                                   object:nil];
        ...
        // Take screenshot and scale 截图  添加毛玻璃效果  应该在这儿加
...
-(void)updateDismissButtonHeight:(CGFloat)height
{
    NSDictionary *options = @{KNSemiModalOptionKeys.pushParentBack : @(NO),
    ...
    UIButton *button = (UIButton *)[overlay viewWithTag:kSemiModalDismissButtonTag];
    // 上一次创建的button
    UIButton *oldButton = [overlay viewWithTag:kSemiModalDismissNewButtonTag];
    
    UIButton *newButton = [UIButton buttonWithType:UIButtonTypeCustom];
    
    newButton.tag = kSemiModalDismissNewButtonTag;
    
    newButton.backgroundColor = [UIColor clearColor];
    
    if (oldButton)
    {
        oldButton.tag = kSemiModalDismissButtonTag;
        newButton.frame = CGRectMake(oldButton.bounds.origin.x, CGRectGetMaxY(oldButton.frame), oldButton.bounds.size.width, height);
    }else
    {
        newButton.frame = CGRectMake(button.bounds.origin.x, CGRectGetMaxY(button.frame), button.bounds.size.width, height);
    }

    [newButton addTarget:self action:@selector(dismissSemiModalView) forControlEvents:UIControlEventTouchUpInside];
