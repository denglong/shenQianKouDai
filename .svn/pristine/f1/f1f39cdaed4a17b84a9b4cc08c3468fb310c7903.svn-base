...
- (CGRect)imageFrameFromImageViewWithAspectFitMode:(UIImageView *)theImageView
{
    ...
    if (imageRatio < viewRatio)
    {
        ...
        return CGRectMake(topLeftX, -25, width, self.frame.size.height);
    }
    else
    {
        ...
        return CGRectMake(0, topLeftY-25, self.frame.size.width, height);
    }
}
...
-(void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if(event.allTouches.count == 1) {
        ...
        NSInteger tapCount = [touch tapCount]; //点击次数
        if(ratioView.frame.size.width*2 > ratioControlsView.frame.size.width){
            return;
        }
        if (tapCount == 2) {
            zoomFactor += 0.4;
            //动画
            ratioView.layer.transform = CATransform3DMakeScale(zoomFactor, zoomFactor, 1.0f);
        } else if (tapCount == 3) {
            zoomFactor += 0.6;
            ratioView.layer.transform = CATransform3DMakeScale(zoomFactor, zoomFactor, 1.0f);
        } else if (tapCount == 4) {
...
//    }
    if(YES == pinchZoom && event.allTouches.count == 2) {
        ...
        if (event.allTouches.count == 1) {
            ...
            if(previousDistance+3 > ratioControlsView.frame.size.width){
                ratioView.frame = CGRectMake(0, ratioView.frame.origin.y,ratioControlsView.frame.size.width, ratioControlsView.frame.size.width);
            }else{
                
                ratioView.layer.transform = CATransform3DMakeScale(zoomFactor, zoomFactor, 1.0f);
            }
        }
        [self.ratioView setFrame:ratioView.frame];
        return;
    }
