...
@synthesize order_name, order_price, orderno, NOTIFY_URL;
...
//    NSString *order_price   = @"1";//1分钱测试
    ...
    [packageParams setObject: self.order_name        forKey:@"body"];        //订单描述，展示给用户
    [packageParams setObject: NOTIFY_URL        forKey:@"notify_url"];  //支付结果异步通知
    [packageParams setObject: self.orderno           forKey:@"out_trade_no"];//商户订单号
    [packageParams setObject: @"127.0.0.1"    forKey:@"spbill_create_ip"];//发起支付的机器ip
    [packageParams setObject: self.order_price       forKey:@"total_fee"];       //订单金额，单位为分
...
- ( NSMutableDictionary *)sendPay_demo:(NSMutableDictionary *)dic {
    ...
    [signParams setObject: mchid        forKey:@"partnerid"];
    [signParams setObject: time_stamp   forKey:@"timestamp"];
    [signParams setObject: [dic objectForKey:@"prepayid"]    forKey:@"prepayid"];
    //[signParams setObject: @"MD5"       forKey:@"signType"];
    //生成签名
    NSString *sign  = [self createMd5Sign:signParams];
